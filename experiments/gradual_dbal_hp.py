...
maxBudgets = ["0.1", "0.2", "0.3", "0.4"]


evaluation_window = ["500"]


tasks  = ["ALPrequentialEvaluationTask"]

evaluators = [ "ALMultiClassImbalancedPerformanceEvaluator"]
...
def train(args):
    ...
    results = [
        (generator, classifier, driftWindow, budget, threshold, max_t, evalWin, max_budget)
    ...
    for (
        ...
        cl_string = (
            "moa.classifiers.active.DBAL -l {} -q {} -r {} -m {} -t {} -f {} -z {}".format(
                classifier, driftWindow, warningWindow, budget, max_t, threshold, max_budget
            )
        )
